...
A aplicação da organização dos dados desta maneira, pode nos oferecer um modelo de analise para compreender o que foi realizado, por exemplo extrair estatisticas, ou para efetuar previsão baseado na tendencia da analise das informações já inseridas.

Há duas maneiras de classificar uma série temporal, determinística quando os valores da série podem ser escritos através de uma função "y = f(tempo)", onde há apenas o tempo como argumento, ou estocástica, quando a série envolve além do tempo, temos aleatórios, "y = f(tempo, ...)".
...
É um aplicativo para facilitar a execução do servidor statim, para executar ele é necessário inidicar um servidor redis para utilizar como storage.
